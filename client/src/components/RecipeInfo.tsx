...
const RecipeInfo: React.FC<RecipeInfoProps> = ({ recipe, ingredients }) => {
  return (
    <article>
      ...
      />
      <Divider />
      <h3 className="text-4xl font-bold text-[#2E5077]">{recipe?.strMeal}</h3>
      <NavLink
        className="text-[#2E5077] hover:text-[#4DA1A9] duration-150 block mt-4 underline"
        to={`/?country=${recipe?.strArea.toLowerCase()}`}
      >
        Country: {recipe?.strArea}
      </NavLink>
      <p className="text-[#2E5077] mt-4 max-w-[400px] min-md:max-w-[600px] min-lg:max-w-[800px]">
